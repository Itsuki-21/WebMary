...
const GOOGLE_SHEETS_URL = 'https://script.google.com/macros/s/AKfycbxHfZjy5XQU5IN0yFCUsUtHDNXqS_Zx3tmB1ZlH1YwKuqrh9e6T7SOUonkoqTYBXCGIDg/exec';
...
function initAdminPanel() {
    ...
    if (viewBookingsBtn) {
        viewBookingsBtn.addEventListener('click', loadBookings);
    }
}
...
function loadBookings() {
    ...
    fetch(CORS_PROXY + encodeURIComponent(GOOGLE_SHEETS_URL), { 
        method: "GET"
    })
                ...
                bookingsList.innerHTML = bookings.map((booking, index) => `
                    <div class="booking-item">
                        <div class="booking-header">
                            ...
                            <span class="booking-date">${formatDateTime(booking.timestamp)}</span>
                        </div>
...
async function submitToGoogleSheets(data, btn) {
    ...
    try {
        ...
        const response = await fetch(CORS_PROXY + encodeURIComponent(GOOGLE_SHEETS_URL), {
            method: 'POST',
            body: formData
        });
